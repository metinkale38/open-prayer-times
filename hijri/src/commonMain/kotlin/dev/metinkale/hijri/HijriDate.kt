...
class HijriDate private constructor(val year: Int, val monthValue: Int, val day: Int) :
    ...
    val month get() = HijriMonth.entries[monthValue - 1]

    fun toLocalDate(): LocalDate {
        return entries.lastOrNull { it.hijri <= hashCode() }?.let { last ->
            LocalDate(last.gy, last.gm, last.gd).plus((day - last.hd).toLong(), DateTimeUnit.DAY)
        } ?: run {
            // we have no data prior 1433, use plattform code
            return Platform.convertFromHijri(this)
        }
    }
    ...
    fun plusDays(days: Int): HijriDate {
        return if (day + days in 1..29) {
            ...
            toLocalDate().plus(days.toLong(), DateTimeUnit.DAY).toHijriDate()
    }

    fun minusDays(days: Int): HijriDate {
        return if (day - days in 1..29) {
            of(year, month, day - days)
        } else toLocalDate().minus(days.toLong(), DateTimeUnit.DAY).toHijriDate()
    }
    ...
    companion object {
        fun getEventsForHijriYear(year: Int): Sequence<Pair<HijriDate, HijriEvent>> = sequence {
            ...
            val ragaib = of(year, HijriMonth.RAJAB, 1).toLocalDate()
                .previousOrSame(DayOfWeek.FRIDAY)
                .toHijriDate()
        ...
        fun getEventsForGregYear(year: Int): Sequence<Pair<HijriDate, HijriEvent>> {
            val min = LocalDate(year, 1, 1).toHijriDate()
            val max = LocalDate(year, 12, 31).toHijriDate()
        ...
        private val entries: List<Entry> by lazy {
            Platform.readFile("/tsv/hijri.tsv")
                .filter { !it.contains("HD") }
                .map { line ->
        ...
        fun fromLocalDate(localDate: LocalDate): HijriDate = localDate.run {
            val greg = year * 10000 + month.number * 100 + day
            return entries.lastOrNull { it.greg <= greg }?.let { last ->
                val ld = LocalDate(last.gy, last.gm, last.gd)
                of(last.hy, last.hm, last.hd - this.daysUntil(ld))
            } ?: run { // we have no data prior 1433, let java convert it
                Platform.convertToHijri(localDate)
            }
        }
    }
